  ...
  async function handleGoogleSheetsSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await createGoogleSheetsSource(accessToken, {
        ...
        sheet_url: googleSheetsForm.sheetUrl.trim() || undefined,
        spreadsheet_id: googleSheetsForm.spreadsheetId.trim() || undefined,
        sheet_gid: googleSheetsForm.sheetGid.trim() || undefined,
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-white/[0.05] border border-white/15 rounded-xl p-4">
            ...
            <input
              ...
              className="h-9 px-3 rounded-lg bg-white/10 border border-white/20 text-sm text-white placeholder:text-white/40 md:col-span-2"
              value={googleSheetsForm.sheetUrl || googleSheetsForm.spreadsheetId}
              onChange={(event) => {
                const value = event.target.value;
                if (value.includes('docs.google.com')) {
                  setGoogleSheetsForm((current) => ({ ...current, sheetUrl: value, spreadsheetId: "" }));
                } else {
                  setGoogleSheetsForm((current) => ({ ...current, sheetUrl: "", spreadsheetId: value }));
                }
              }}
              required
            />
            <input
              type="text"
              placeholder="Sheet GID (optional, auto-detected from URL)"
              className="h-9 px-3 rounded-lg bg-white/10 border border-white/20 text-sm text-white placeholder:text-white/40"
