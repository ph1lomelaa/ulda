...
def parse_google_sheets_url(url: str) -> tuple[str, str]:
    """
    Parse Google Sheets URL to extract spreadsheet ID and sheet GID.
    
    Supports formats:
    - https://docs.google.com/spreadsheets/d/{spreadsheet_id}/edit#gid={gid}
    - https://docs.google.com/spreadsheets/d/{spreadsheet_id}/edit?usp=sharing#gid={gid}
    - https://docs.google.com/spreadsheets/d/{spreadsheet_id}/edit
    
    Returns:
        tuple: (spreadsheet_id, sheet_gid)
    
    Raises:
        ValueError: If URL format is invalid or IDs cannot be extracted
    """
    clean_url = url.strip()
    
    # Extract spreadsheet ID
    spreadsheet_match = re.search(r'/spreadsheets/d/([a-zA-Z0-9-_]+)', clean_url)
    ...
    # Extract GID (default to 0 if not found)
    gid_match = re.search(r'[#&]gid=([0-9]+)', clean_url)
    sheet_gid = gid_match.group(1) if gid_match else "0"
