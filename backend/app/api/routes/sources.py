...
def create_google_sheets_source(
    ...
    # Parse URL if provided, otherwise use direct IDs
    if payload.sheet_url and payload.sheet_url.strip():
        try:
            from app.services.google_sheets_connector import parse_google_sheets_url
            spreadsheet_id, sheet_gid = parse_google_sheets_url(payload.sheet_url)
        except ValueError as exc:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    else:
        if not payload.spreadsheet_id or not payload.spreadsheet_id.strip():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Either sheet_url or spreadsheet_id is required")
        spreadsheet_id = payload.spreadsheet_id.strip()
        sheet_gid = (payload.sheet_gid or "0").strip()
    ...
    display_name = payload.sheet_name.strip() if payload.sheet_name else spreadsheet_id
    document = Document(
        source_id=source.id,
    ...
    create_audit_log(
        ...
        entity_type="data_source",
        entity_id=source.id,
        detail=f"{spreadsheet_id}:{sheet_gid}",
    )
    db.commit()
